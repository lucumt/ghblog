...
由于线程B以读取`volatile`变量sharedObject.counter开始，因而变量sharedObject.counter和变量sharedObject.nonVolatile都会被写入线程B所使用的CPU缓存中。当线程B读取sharedObject.nonVolatile变量时，它将能看见被线程A写入的变量。  
